...
linestyles = cellstr(char('-',':','-.','--','-',':','-.','--','-',':','-',':',...
    '-.','--','-',':','-.','--','-',':','-.','-',':','-.','--','-',':','-.','--'));
MarkerEdgeColors = colormap('jet');
step = floor(size(MarkerEdgeColors,1)/size(Data1,1));
...
for i = 1:rows
    ...
    stairs((v),(f),'DisplayName',Algos{i},'color',MarkerEdgeColors(count,:),'linestyle',linestyles{i},'LineWidth',2); hold on;grid on;% 
    count = count + step;
end
set(gca, 'Xlim', [0 ceil(v_max)],'XScale', 'log');%'XScale', 'log' ,
